...
namespace viperOSK
{
    public class OSK_UI_Cursor : MonoBehaviour, I_OSK_Cursor
    {
        ...
        {
            if (input == null)
                input = this.GetComponentInParent<OSK_Receiver>();
            ...
            if (cursorImg == null)
                cursorImg = this.GetComponent<Image>();
        ...
        { 
            ...
            startingCursorPos.y = (textComponent.textInfo.characterInfo[0].vertex_BR.position.y+ textComponent.textInfo.characterInfo[0].vertex_TR.position.y)*.5f;
            textComponent.text = "";
        ...
        public void Cursor()
        {
            ...
            else if (input.cursorIndex >= 0 && textInfo.characterCount > 0)
            {
                ...
                cursorPos.y = (charInfo.ascender + charInfo.descender) * .5f;  //(charInfo.vertex_BR.position.y + charInfo.vertex_TR.position.y) * .5f;

            }

            this.GetComponent<RectTransform>().localPosition = new Vector3(cursorPos.x + cursorImg.rectTransform.rect.width * .5f, cursorPos.y, cursorPos.z);
        }
